...
Turn **`spec.md` intent** into a **small, reviewable change bundle** so implementers and reviewers see scope and sequencing without reading the whole repo. Output: **proposal**, **tasks** (split for Code, single for Infra), optional **design** — kept short and tied to the source spec.
...
- Read the confirmed **`spec.md`** at the provided path — if the file does not exist, **stop**: hand off to **ssot-spec-writer** to draft it first.
- Derive the **platform** (`folklore` or `authpt`), **domain**, and **capability** from the spec path.
...
- **Read each linked artifact** (events schema, DB model, API contract) — extract names, key fields, and config values needed to write concrete implementation tasks. Do not copy schemas verbatim; extract only what drives task wording (e.g. JPA entity name, controller class, SNS topic name, SQS queue, `application.properties` key, repository class, interceptor name, publisher event type).
...
- **Read the `> **Type:**` header of the input spec** — it is either `Code` or `Infra`. This drives the entire shape of the output:
  ...
  - **Infra spec** → scope is CloudFormation resources and minimal scaffold only; `infra-tasks.md` has phases `## Spec & Alignment` → `## Infrastructure`. No `## Implementation` or `## Testing` phases.
...
4. **Scope / Non-goals** — explicit bullets, derived from the spec `Type`:
   - **Code spec**: scope is business logic, MVC, Service Layer, Repository, DTO, Builder, Template Method, Strategy, Specification, Chain of Responsibility, Observer/Event-Driven, Facade, Interface-Implementation, Factory Method, Proxy/Client Adapter, Decorator, JPA Inheritance (Joined), Singleton (Spring scope), Retry, Scheduled Task, Validator and Test Builder/Object Mother. Infrastructure provisioning is **out of scope** (covered by the companion Infra spec).
   - **Infra spec**: scope is CloudFormation resources (ECS task/service definitions, CloudWatch alarms, IAM roles) and the deploy layer (`run.py` + `daasdeployhelpers`). Business logic is **out of scope** (covered by the companion Code spec).
...
6. **Artifacts** — start with the standard deferral note: *"Artifacts are listed in `./spec.md#references`; verify all expected paths exist before starting implementation."* Then **explicitly list every new file the change will create** (e.g. `Controller.java`, `Service.java`, `ServiceImpl.java`, `Repository.java`, `Request.java`, `Response.java`, `ExceptionHandler.java`, `Publisher.java`, `PublisherImpl.java`, `PublisherBuilder.java`, `Listener.java`, `Handler.java`, `Config.java`, `Interceptor.java`, `Validator.java`) with a one-line description of its purpose. Do not repeat paths already in the spec's `## References`; only add paths for outputs that do not exist there.
...
- **Granularity**: tasks must name **specific files, classes, packages, or configuration keys** — not just paraphrase a requirement. Use values extracted from artifacts in Phase 1 (e.g. JPA entity name, controller class, SNS topic name, SQS queue, `application.properties` key, repository class, interceptor name, publisher event type).
...
- Under each phase: checkbox tasks `- [ ]` that are **single-step** and **verifiable**.
- **Granularity**: tasks must name **specific CloudFormation resources, IAM roles, CloudWatch alarms, environment variables, or deploy-script entries** — e.g. ECS task definition name, service name, target group, `run.py` config block, profile-based property file.
- **Traceability** requirements match the Code task files.
...
- [ ] `proposal.md` Scope matches the input spec `Type`: business logic, controllers, services, repositories, publishers, consumers, and validators for Code specs; Spring Boot configuration classes, AWS client beans, interceptor configs, Feign client wiring, and profile-based property files for Infra specs
...
If planning reveals new **MUST** statements or missing scenarios, note them in `proposal.md` under **Risks / Open questions** and add a task in the `## Spec & Alignment` phase. The **ssot-spec-writer** agent should update `spec.md` before or alongside implementation.
...
- Creating output files when no `spec.md` exists for domain-scoped work.
